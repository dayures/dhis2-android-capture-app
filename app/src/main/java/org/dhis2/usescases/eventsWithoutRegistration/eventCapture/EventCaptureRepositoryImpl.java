...
public class EventCaptureRepositoryImpl implements EventCaptureContract.EventCaptureRepository {
    ...
    private static final String SELECT_SECTIONS = "SELECT\n" +
            ...
            "WHERE Event.uid = ?\n" +
            "AND " + EventModel.TABLE + "." + EventModel.Columns.STATE + NOT_EQUALS + QUOTE + State.TO_DELETE + "' ORDER BY ProgramStageSection.sortOrder";
    ...
    public boolean isEnrollmentCancelled() {
        ...
        try (Cursor enrollmentCursor = briteDatabase.query("SELECT Enrollment.* FROM Enrollment JOIN Event ON Event.enrollment = Enrollment.uid WHERE Event.uid = ?", eventUid)) {
            if (enrollmentCursor != null && enrollmentCursor.moveToFirst()) {
                Enrollment enrollment = Enrollment.create(enrollmentCursor);
                isEnrollmentCancelled = enrollment.status() == EnrollmentStatus.CANCELLED;
            }
        }
    ...
    public Flowable<String> programStageName() {
        return briteDatabase.createQuery(ProgramStageModel.TABLE,
                "SELECT ProgramStage.* FROM ProgramStage " +
                        ...
                        WHERE + EventModel.TABLE + "." + EventModel.Columns.UID + EQUAL + QUESTION_MARK + LIMIT_1, eventUid)
                ...
                .flatMap(programUid -> {
                    ...
                    if (!isEmpty(st))
                        return briteDatabase.createQuery(ProgramRuleModel.TABLE,
                                ...
                                .mapToList(cursor -> {
                                    ProgramRule ruleModel = ProgramRule.create(cursor);
                                    ...
                                    try (Cursor actionsCursor = briteDatabase.query(
                                        ...
                                        if (actionsCursor != null && actionsCursor.moveToFirst()) {
                                    ...
                                    return Rule.create(ruleModel.programStage().uid(), ruleModel.priority(),
                                            ruleModel.condition(), ruleActions, ruleModel.displayName());
                                });
                    else
                    ...
                    try (Cursor hideRulesCursor = briteDatabase.query("SELECT ProgramRule.* FROM ProgramRule " +
                        ...
                        if (hideRulesCursor != null && hideRulesCursor.moveToFirst()) {
                            for (int i = 0; i < hideRulesCursor.getCount(); i++) {
                                ProgramRule ruleModel = ProgramRule.create(hideRulesCursor);
                                ruleMap.put(ruleModel.displayName(), Rule.create(ruleModel.programStage().uid(),
                                        ruleModel.priority(), ruleModel.condition(), getRuleActionsFor(ruleModel.uid()),
                                        ruleModel.displayName()));
    ...
    public Observable<Boolean> completeEvent() {
        ...
        return Observable.just(briteDatabase.update(EventModel.TABLE, contentValues, UID_EQUALS_QUESTION_MARK, eventUid) > 0);
    }
    ...
    public boolean reopenEvent() {
        ...
        return briteDatabase.update(EventModel.TABLE, contentValues, UID_EQUALS_QUESTION_MARK, eventUid) > 0;
    }
    ...
    private void updateEnrollment(String enrollmentUid) {
        ...
        try (Cursor enrollmentCursor = briteDatabase.query(selectEnrollment, enrollmentUid)) {
            if (enrollmentCursor != null && enrollmentCursor.moveToFirst()) {
                ...
                briteDatabase.update(EnrollmentModel.TABLE, cv, UID_EQUALS_QUESTION_MARK, enrollmentUid);
    ...
    private void updateTei(String teiUid) {
        ...
        try (Cursor teiCursor = briteDatabase.query(selectTei, teiUid)) {
            if (teiCursor != null && teiCursor.moveToFirst()) {
                ...
                briteDatabase.update(TrackedEntityInstanceModel.TABLE, cv, UID_EQUALS_QUESTION_MARK, teiUid);
            }
        }
    }
    ...
    public Observable<Boolean> rescheduleEvent(Date newDate) {
        ...
        return Observable.just(briteDatabase.update(EventModel.TABLE, contentValues, UID_EQUALS_QUESTION_MARK, eventUid))
