...
public class EventSummaryRepositoryImpl implements EventSummaryRepository {
    ...
    public Observable<Event> changeStatus(String eventUid) {
        ...
        try (Cursor cursor = briteDatabase.query(EVENT_QUERY, eventUid)) {
            if (cursor != null && cursor.moveToNext()) {
                ...
                ContentValues values = getContentValues(event, lastUpdated);
                updateEvent(event, values);
                updateProgram(cursor, values, lastUpdated, event);
    ...
    private void updateEvent(Event event, ContentValues values) {
    ...
    private ContentValues getContentValues(Event event, String lastUpdated) {
    ...
    private void updateProgram(Cursor cursor, ContentValues values, String lastUpdated, Event event) {
    ...
    public Flowable<Event> getEvent(String eventId) {
        return briteDatabase.createQuery(SqlConstants.EVENT_TABLE, EVENT_QUERY, eventId == null ? "" : eventId)
                .mapToOne(Event::create).toFlowable(BackpressureStrategy.LATEST);
    }

    @Override
    public Observable<Boolean> accessDataWrite(String eventId) {
        return briteDatabase.createQuery(SqlConstants.PROGRAM_STAGE_TABLE, ACCESS_QUERY, eventId == null ? "" : eventId)
                .mapToOne(cursor -> cursor.getInt(0) == 1)
                .flatMap(programStageAccessDataWrite -> briteDatabase.createQuery(SqlConstants.PROGRAM_TABLE, PROGRAM_ACCESS_QUERY, eventId == null ? "" : eventId)
